...
#include <sys/stat.h>
#include <chrono>
static long long _diff_ns = 0, _cell_ns = 0;
...
int main(int argc, char* argv[]) {
    ...
    for (int i_episode = 0; i_episode < 4; i_episode++) {
        ...
        try {
            ...
            while (step) {
                ...
                { auto _t0=std::chrono::high_resolution_clock::now();
                  ...
                  _diff_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now()-_t0).count(); }

                // run PhysiCell
                { auto _t0=std::chrono::high_resolution_clock::now();
                  ((Cell_Container *)microenvironment.agent_container)->update_all_cells (PhysiCell_globals.current_time);
                  _cell_ns += std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::high_resolution_clock::now()-_t0).count(); }
        ...
        std::cout << std::endl;
        double _diff_s = _diff_ns * 1e-9;
        double _cell_s = _cell_ns * 1e-9;
        double _total_s = _diff_s + _cell_s;
        std::printf("\n=== TIMING SPLIT (episode %d) ===\n", i_episode);
        std::printf("  diffusion (simulate_diffusion_decay): %.3f s  (%.1f%%)\n", _diff_s,  100.0*_diff_s/_total_s);
        std::printf("  cells     (update_all_cells):         %.3f s  (%.1f%%)\n", _cell_s,  100.0*_cell_s/_total_s);
        std::printf("  tracked total:                        %.3f s\n", _total_s);
        std::printf("================================\n\n");
        _diff_ns = _cell_ns = 0;
