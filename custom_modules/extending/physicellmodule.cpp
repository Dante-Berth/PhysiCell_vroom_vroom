...
// runtime verbosity: when the PHYSIGYM_QUIET environment variable is set (to
// anything non-empty other than "0"), suppress PhysiCell's C++ console chatter
// (banners, per-step "current simulated time / total agents" status, etc.).
// The Python physigym layer sets this from its own `verbose` flag so the C++
// output matches the Python verbosity instead of always printing.
static bool physigym_quiet() {
    const char* v = std::getenv("PHYSIGYM_QUIET");
    return v != nullptr && v[0] != '\0' && !(v[0] == '0' && v[1] == '\0');
}

// RAII guard: while quiet, redirect std::cout into a real /dev/null sink and
// restore on scope exit (covers every return path). A valid streambuf is used
// rather than nullptr: setting std::cout's buffer to nullptr leaves the stream
// in a state where downstream insertions/flushes inside PhysiCell can crash.
static std::ofstream& cout_null_sink() {
    static std::ofstream sink("/dev/null");
    return sink;
}
struct CoutMuteGuard {
    std::streambuf* saved;
    CoutMuteGuard() : saved(nullptr) {
        if (physigym_quiet() && cout_null_sink().is_open()) {
            saved = std::cout.rdbuf(cout_null_sink().rdbuf());
        }
    }
    ~CoutMuteGuard() { if (saved) { std::cout.rdbuf(saved); } }
};
...
static PyObject* physicell_start(PyObject *self, PyObject *args) {
    ...
    // suppress PhysiCell C++ console output when physigym runs non-verbose
    CoutMuteGuard _cout_mute;
    ...
    if (! reload) {
        ...
        // reset cells
        // ACHTUNG: die() -> delete_cell() mutates (*all_cells) in place (it swaps
        // the doomed cell with the last element and pop_back()s). Iterating with a
        // range-based for over (*all_cells) while it is being mutated is undefined
        // behavior: the end iterator goes stale, cells get skipped, and a freed /
        // out-of-range Cell* is dereferenced -> segfault in release_internalized_
        // substrates (stale current_voxel_index). Repeatedly delete the back element
        // instead, which delete_cell handles in O(1) without invalidating the rest.
    ...
    //try {
        ...
        while (step) {
            ...
            if (action) {
                ...
                // Execute the targeted dose and capture the total mass administered
